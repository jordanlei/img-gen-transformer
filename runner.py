...
class Runner:
    def __init__(self, model, optimizer, device, teacher_forcing=False):
        self.model = model
    ...
    def train(self, train_loader, epochs):
        ...
        for epoch in progress_bar:
            for batch in train_loader:
                ...
                recon_x, mu, logvar, cls_output = self.model(x, cls_input)

                # VAE losses
                recon_loss = F.mse_loss(recon_x, x, reduction="sum") / x.shape[0]
                kl_loss = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp()) / x.shape[0]
                class_loss = F.cross_entropy(cls_output, y)
                ...
                steer_loss = F.cross_entropy(steer_cls_output, random_classes)

                # Total loss
                total_loss = recon_loss + kl_loss + class_loss + 50 * steer_loss
                total_loss.backward()
                self.optimizer.step()
